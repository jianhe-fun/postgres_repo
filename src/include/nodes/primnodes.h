...
typedef struct SafeTypeCastExpr
{
	...
	/*
	 * The parsed and transformed source expression.
	 *
	 * This is used for deparsing only. In cases like "CAST(1 AS date DEFAULT
	 * NULL ON CONVERSION ERROR)", where the castexpr evaluates to NULL, we
	 * need this field to reconstruct the original query.
	 */
	Expr	   *source;
