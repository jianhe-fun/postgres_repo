						 ...
						 ExprEvalStep *scratch)
{
	...
	else
	{
		...
		/*
		 * Evaluate the DEFAULT expression, skipping if it's a binary coercion
		 * cast. state->escontext must be NULL, as this evaluation should not
		 * be error-safe.
		 */
		if (!IsA(stcexpr->castexpr, RelabelType))
		{
