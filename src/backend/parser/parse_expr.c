	...
	foreach(element, newelems)
	{
		...
		if (coerce_hard)
		{
			...
			else
			{
				...
				else if (is_coercible && IsA(ecopy, Const) && IsA(newe, FuncExpr))
				{
					Node	   *newexpr = copyObject(newe);

					/*
					 * Pre-evaluate simple constant cast expressions in a way
					 * that tolerate errors.
					 */
					assign_expr_collations(pstate, newexpr);

					newexpr = (Node *) evaluate_expr((Expr *) newexpr,
													 coerce_type,
													 typmod,
													 coerce_type_coll,
													 (Node *) &escontext);
...
transformTypeCast(ParseState *pstate, TypeCast *tc)
{
	...
	if (IsA(source, Const) && castexpr && IsA(castexpr, FuncExpr))
	{
		Node	   *newexpr = copyObject(castexpr);

		assign_expr_collations(pstate, newexpr);

		/*
		 * Pre-evaluate simple constant cast expressions in an error-safe
		 * manner via evaluate_expr.
		 *
		 * Rationale: 1. Later operations, such as deparsing safe-cast
		 * expressions, may invoke eval_const_expressions(). Because that
		 * function cannot handle errors gracefully, we preemptively perform
		 * the evaluation here using the error-safe evaluate_expr() instead.
		 *
		 * 2. Even if the FuncExpr (for castfunc) has three arguments, the
		 * second and third arguments are guaranteed to be constants (see
		 * coerce_to_target_type). Evaluating a FuncExpr where all arguments
		 * are Const nodes is safe in evaluate_expr.
		 */
		newexpr = (Node *) evaluate_expr((Expr *) newexpr,
										 targetType,
										 targetTypmod,
						   ...
						   bool *errorsafe_coercion, bool *userdefined)
{
	...
	{
		/*
		 * Input expression can be UNKNOWN Const, and UNKNOWN is typtype is
		 * TYPTYPE_PSEUDO
		 */
	...
	/*
	 * Casts involving type MONEY are not error safe.  Type CIRCLE to type
	 * POLYGON cast is unsafe because it relies on a SQL-language function;
	 * only C-language functions are currently supported for error safe casts.
	 */
	if (inputType == MONEYOID ||
		targetType == MONEYOID ||
		(inputType == CIRCLEOID && targetType == POLYGONOID))
	{
		*errorsafe_coercion = false;
		return;
	}
	...
	if (inputType != targetType)
	{
		...
		if (HeapTupleIsValid(tuple))
		{
			...
			if (castForm->oid > FirstUnpinnedObjectId &&
				castForm->castmethod != COERCION_METHOD_BINARY)
			{
				*errorsafe_coercion = false;
				*userdefined = true;
			}
